...
/**
 * Default required section labels used when no explicit list is provided.
 * Mirrors the legacy Scalian template contract; ports that have a richer
 * template manifest should pass `requiredSections` explicitly.
 */
...
export async function validateDocxBuffer(
  ...
  // Check required sections from the caller-supplied list, or fallback to legacy defaults.
  const required = requiredSections && requiredSections.length > 0
    ? requiredSections
